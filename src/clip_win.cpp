...
class Hglobal {
  public:
    ...
    void release() { m_handle = nullptr; }

    operator HGLOBAL() { return m_handle; }
...
struct BitmapInfo {
    ...
    BitmapInfo() {
        ...
        if (IsClipboardFormatAvailable(CF_DIBV5)) {
            ...
            if (b5 && b5->bV5BitCount == 32 &&
                ((b5->bV5Compression == BI_RGB) ||
                 (b5->bV5Compression == BI_BITFIELDS && b5->bV5RedMask &&
                  b5->bV5GreenMask && b5->bV5BlueMask && b5->bV5AlphaMask))) {
                width = b5->bV5Width;
                height = b5->bV5Height;
    ...
    bool is_valid() const { return (b5 || bi); }
    ...
    void fill_spec(image_spec &spec) {
        ...
        for (unsigned long *shift = shifts, *mask = masks; shift < shifts + 4;
             ++shift, ++mask) {
            if (*mask)
...
bool lock::impl::clear() { return (EmptyClipboard() ? true : false); }
    ...
    if (f == text_format()) {
        if (len > 0) {
            ...
            if (reqsize > 0) {
                ...
                result =
                    (SetClipboardData(CF_UNICODETEXT, hglobal)) ? true : false;
                if (result)
                    hglobal.release();
            }
        }
    ...
    if (f == text_format()) {
        if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
            ...
            if (hglobal) {
                LPWSTR lpstr = static_cast<LPWSTR>(GlobalLock(hglobal));
                if (lpstr) {
                    size_t reqsize = WideCharToMultiByte(
                        CP_UTF8, 0, lpstr, -1, nullptr, 0, nullptr, nullptr);

                    assert(reqsize <= len);
                    if (reqsize <= len) {
                        WideCharToMultiByte(CP_UTF8, 0, lpstr, -1, buf, reqsize,
                                            nullptr, nullptr);
                        result = true;
                    }
                    GlobalUnlock(hglobal);
    ...
    if (f == text_format()) {
        if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
            ...
            if (hglobal) {
                LPWSTR lpstr = static_cast<LPWSTR>(GlobalLock(hglobal));
                if (lpstr) {
                    len = WideCharToMultiByte(CP_UTF8, 0, lpstr, -1, nullptr, 0,
                                              nullptr, nullptr);
                    GlobalUnlock(hglobal);
                }
            }
...
bool lock::impl::set_image(const image &image) {
    ...
    if (spec.bits_per_pixel == 32 && spec.alpha_mask) {
    ...
    switch (spec.bits_per_pixel) {
    case 24:
        padding = (4 - ((spec.width * 3) & 3)) & 3;
        break;
    case 16:
        padding = ((4 - ((spec.width * 2) & 3)) & 3) / 2;
        break;
    case 8:
        padding = (4 - (spec.width & 3)) & 3;
        break;
    }
    ...
    Hglobal hmem(GlobalAlloc(
        GHND, sizeof(BITMAPV5HEADER) + palette_colors * sizeof(RGBQUAD) +
    ...
    case 32: {
        ...
        for (long y = spec.height - 1; y >= 0; --y) {
            ...
            for (unsigned long x = 0; x < spec.width; ++x, ++src_x, ++dst_x) {
                ...
                *dst_x =
                    (r << out_spec.red_shift) | (g << out_spec.green_shift) |
                    (b << out_spec.blue_shift) | (a << out_spec.alpha_shift);
            }
...
bool lock::impl::get_image(image &output_img) const {
    ...
    if (png_format && IsClipboardFormatAvailable(png_format)) {
        ...
        if (png_handle) {
            ...
            bool result =
                win::read_png(png_data, png_size, &output_img, nullptr);
    ...
    case 16: {
        ...
        if (bi.compression == BI_RGB || bi.compression == BI_BITFIELDS) {
            if (bi.b5)
        ...
        if (src) {
            ...
            for (long y = spec.height - 1; y >= 0;
                 --y, src += src_bytes_per_row + padding) {
    ...
    case 8: {
        ...
        const int colors =
            (bi.bi->bmiHeader.biClrUsed > 0 ? bi.bi->bmiHeader.biClrUsed : 256);
        std::vector<uint32_t> palette(colors);
        for (int c = 0; c < colors; ++c) {
            palette[c] = (bi.bi->bmiColors[c].rgbRed << spec.red_shift) |
                         (bi.bi->bmiColors[c].rgbGreen << spec.green_shift) |
                         (bi.bi->bmiColors[c].rgbBlue << spec.blue_shift);
        }

        const uint8_t *src = (((uint8_t *)bi.bi) + bi.bi->bmiHeader.biSize +
...
format register_format(const std::string &name) {
    int reqsize =
        1 + MultiByteToWideChar(CP_UTF8, 0, name.c_str(), name.size(), NULL, 0);
    std::vector<WCHAR> buf(reqsize);
    MultiByteToWideChar(CP_UTF8, 0, name.c_str(), name.size(), &buf[0],
                        reqsize);
